...
sectorsRouter.get("/api/advance-decline", async (req, res) => {
  ...
  try {
    // ^ADVN/^DECN/^ADDQ/^DCLQ: Yahoo breadth indices
    // safeChart filters close>0 but these tickers use close as count (can be 0)
    // Use a relaxed fetch that keeps all bars
    const fetchBreadthChart = async (sym, daysN) => {
      try {
        const period1 = new Date(Date.now() - (daysN+30) * 86_400_000);
        const r = await withTimeout(yf.chart(sym, { period1, interval: "1d" }), 15_000);
        const valid = (r?.quotes || []).filter(q => q.close != null && q.date);
        return {
          closes:     valid.map(q => Math.round(q.close || 0)),
          timestamps: valid.map(q => Math.floor(new Date(q.date).getTime() / 1000)),
        };
      } catch(e) {
        log.debug(`breadthChart(${sym}): ${e.message}`);
        // Fallback: use yf.quote for today's snapshot only
        try {
          const q = await withTimeout(yf.quote(sym), 8_000);
          const price = q?.regularMarketPrice || q?.postMarketPrice || 0;
          if (price > 0) return {
            closes: [Math.round(price)],
    ...
    const [advnData, decnData, addqData, dclqData, spyChart] = await Promise.allSettled([
      fetchBreadthChart("^ADVN", days),
      fetchBreadthChart("^DECN", days),
      fetchBreadthChart("^ADDQ", days),
      fetchBreadthChart("^DCLQ", days),
    ...
    const buildADFromCounts = (advRes, decRes) => {
      ...
      const n    = Math.min(days, adv.closes.length, dec.closes.length);
      const advS = adv.closes.slice(-n);
      const decS = dec.closes.slice(-n);
      const tsS  = adv.timestamps.slice(-n);
      let cum = 0;
      const raw = [], cumLine = [];
      for (let i = 0; i < n; i++) {
        const a = Math.round(advS[i] || 0), d = Math.round(decS[i] || 0);
        const net = a - d;
        cum += net;
        const ratio = d > 0 ? +(a/d).toFixed(2) : (a > 0 ? 9.99 : 1.00);
        const advPct = (a+d) > 0 ? +((a/(a+d))*100).toFixed(1) : null;
        raw.push({ ts: tsS[i], value: net, net, adv: a, dec: d, ratio, advPct });
        cumLine.push({ ts: tsS[i], value: +cum.toFixed(0) });
      }
      const lastAdv = advS[advS.length-1] ?? 0;
      const lastDec = decS[decS.length-1] ?? 0;
      return {
        raw, cumulative: cumLine,
        latest:    raw[raw.length-1]?.net ?? 0,
        latestCum: cum,
        latestAdv: Math.round(lastAdv),
        latestDec: Math.round(lastDec),
        latestRatio: lastDec > 0 ? +(lastAdv/lastDec).toFixed(2) : (lastAdv > 0 ? 9.99 : null),
      };
    };

    const nyseAD   = buildADFromCounts(advnData, decnData);
    const nasdaqAD = buildADFromCounts(addqData, dclqData);
